...
export async function GET(request: Request) {
  try {
    ...
    const [users, profiles, questions, sessions, winners, gameModes, questionRules] = await Promise.all([
      adminAuth().listUsers(), adminDb().collection('users').get(), adminDb().collection('questions').get(), adminDb().collection('sessions').get(), adminDb().collection('winners').get(), adminDb().collection('platformSettings').doc('gameModes').get(), adminDb().collection('platformSettings').doc('questionRules').get(),
    ]);
    ...
    }, gameModes: gameModes.data()?.enabled || {}, questionCategories: [...new Set(questions.docs.map((question) => String(question.data().category || '').trim()).filter(Boolean))].sort((a, b) => a.localeCompare(b, 'ar')), questionRules: questionRules.data()?.rules || {} });
...
export async function PATCH(request: Request) {
  try {
    await requireAdmin(request);
    const { uid, action, password, gameModes, questionRules } = await request.json() as { uid?: string; action?: 'enable' | 'disable' | 'reset-password' | 'set-game-visibility' | 'set-game-question-rules'; password?: string; gameModes?: Record<string, boolean>; questionRules?: Record<string, { categories?: string[]; questionTypes?: string[] }> };
