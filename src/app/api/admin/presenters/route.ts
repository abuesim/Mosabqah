...
export async function GET(request: Request) {
  try {
    ...
    const [
      ...
      adminDb().collection("winners").get(),
    ...
    const profilesByUid = new Map(
      profiles.docs.map((profile) => [profile.id, profile.data()]),
    );
    const managedUsers = users.users
      .map((user) => ({
        uid: user.uid,
        username:
          profilesByUid.get(user.uid)?.username ||
          user.displayName ||
          user.email?.replace(/@mosabqah\.local$/, "") ||
          "مستخدم",
        role:
          user.customClaims?.admin === true
            ? "admin"
            : profilesByUid.get(user.uid)?.role === "player"
              ? "player"
              : "presenter",
    ...
    return NextResponse.json({
      users: managedUsers,
      stats: {
        presenters: managedUsers.filter((user) => user.role === "presenter")
          .length,
      ...
      gameModes: gameModes.data()?.enabled || {},
      questionCategories: [
        ...new Set(
          questions.docs
            .map((question) => String(question.data().category || "").trim())
            .filter(Boolean),
        ),
      ].sort((a, b) => a.localeCompare(b, "ar")),
      questionRules: questionRules.data()?.rules || {},
      gameInstructions: gameInstructions.data()?.instructions || {},
    });
  } catch (error) {
...
export async function POST(request: Request) {
  try {
    ...
    if (!username?.trim() || !password || password.length < 6)
      throw new Error("أدخل اسم مستخدم وكلمة مرور من 6 أحرف على الأقل.");
    if (role && !["presenter", "player"].includes(role))
  ...
  try {
    ...
    if (action === "set-game-visibility") {
      ...
      await adminDb()
        ...
        .doc("gameModes")
        .set(
          { enabled: gameModes, updatedAt: FieldValue.serverTimestamp() },
          { merge: true },
        );
      return NextResponse.json({ ok: true });
    }
    if (action === "set-game-question-rules") {
      if (!questionRules || typeof questionRules !== "object")
        throw new Error("إعدادات بنك الأسئلة غير مكتملة.");
      const rules = Object.fromEntries(
        Object.entries(questionRules).map(([game, rule]) => [
          game,
          {
            categories: [
              ...new Set(
            ...
            questionTypes: [
              ...new Set(
                (rule.questionTypes || []).filter((value): value is string =>
                  ["text", "image", "word"].includes(value),
                ),
              ),
            ],
            bankEnabled: rule.bankEnabled !== false,
          },
        ]),
      );
      await adminDb()
        ...
        .doc("questionRules")
        .set(
          { rules, updatedAt: FieldValue.serverTimestamp() },
          { merge: true },
        );
      return NextResponse.json({ ok: true });
    }
    if (action === "set-game-instructions") {
      if (!gameInstructions || typeof gameInstructions !== "object")
        throw new Error("شروحات الألعاب غير مكتملة.");
      const instructions = Object.fromEntries(
        Object.entries(gameInstructions).map(([game, text]) => [
          game,
          String(text || "")
            .trim()
            .slice(0, 4000),
      ...
      await adminDb()
        .collection("platformSettings")
        .doc("gameInstructions")
        .set(
          { instructions, updatedAt: FieldValue.serverTimestamp() },
          { merge: true },
        );
      return NextResponse.json({ ok: true });
    }
    ...
    if (target.customClaims?.admin === true)
      throw new Error("لا يمكن تعديل حساب مدير النظام من هذه الصفحة.");
