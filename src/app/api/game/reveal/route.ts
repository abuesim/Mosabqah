...
export async function POST(request: Request) {
  try {
    ...
    if (!session.exists || session.data()?.createdBy !== user.uid) return NextResponse.json({ error: 'غير مصرح لك بكشف إجابة هذه الجلسة.' }, { status: 403 });
    return NextResponse.json(await (session.data()?.gameMode === 'chairs'
      ? revealAndScoreChairRound(sessionId, questionId)
      : revealAndScoreQuestion(sessionId, questionId)));
  } catch (error) {
    const message = error instanceof Error ? error.message : 'تعذر كشف الإجابة.';
