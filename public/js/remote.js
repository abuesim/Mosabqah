document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderQuestionsPool() {
    ...
    [...unasked, ...asked].forEach(q => {
      ...
      const catBadgeHtml = `<span style="background: ${catStyle.bg}; color: ${catStyle.color}; padding: 2px 6px; border-radius: 12px; font-size: 10px; font-weight: bold; border: 1px solid ${catStyle.border || 'rgba(255, 255, 255, 0.15)'};">${catStyle.text}</span>`;
      ...
      const diffBadgeHtml = `<span style="background: ${diffStyle.bg}; color: ${diffStyle.color}; padding: 2px 6px; border-radius: 12px; font-size: 10px; font-weight: bold; border: 1px solid ${diffStyle.border || 'rgba(255, 165, 2, 0.3)'};">${diffStyle.text}</span>`;
